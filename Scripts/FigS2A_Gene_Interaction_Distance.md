...
# Count the number of genes within different distances
Matrix <- matrix(ncol=22, nrow=10)
Matrix[,1] <- c(50000,100000,150000,200000,250000,300000,350000,400000,450000,500000)
for(i in 1:10) { 
  Matrix[i,2] <- nrow(InducedGenes[ InducedGenes$Symbol %in% InducedInt[ abs(InducedInt$dist) <= Matrix[i,1],"Symbol"],])
  Matrix[i,3] <- nrow(InducedGenes[ InducedGenes$Symbol %in% RepressedInt[ abs(RepressedInt$dist) <= Matrix[i,1],"Symbol"],])
  Matrix[i,4] <- nrow(InducedGenes[ InducedGenes$Symbol %in% FilteredInteractions_Genes[ abs(FilteredInteractions_Genes$dist) <= Matrix[i,1],"Symbol"],])
  Matrix[i,5] <- nrow(RepressedGenes[ RepressedGenes$Symbol %in% InducedInt[ abs(InducedInt$dist) <= Matrix[i,1],"Symbol"],])
  Matrix[i,6] <- nrow(RepressedGenes[ RepressedGenes$Symbol %in% RepressedInt[ abs(RepressedInt$dist) <= Matrix[i,1],"Symbol"],])
  Matrix[i,7] <- nrow(RepressedGenes[ RepressedGenes$Symbol %in% FilteredInteractions_Genes[ abs(FilteredInteractions_Genes$dist) <= Matrix[i,1],"Symbol"],])
  Matrix[i,8] <- nrow(RNA[ RNA$Symbol %in% InducedInt[ abs(InducedInt$dist) <= Matrix[i,1],"Symbol"],])
  Matrix[i,9] <- nrow(RNA[ RNA$Symbol %in% RepressedInt[ abs(RepressedInt$dist) <= Matrix[i,1],"Symbol"],])
  Matrix[i,10] <- nrow(RNA[ RNA$Symbol %in% FilteredInteractions_Genes[ abs(FilteredInteractions_Genes$dist) <= Matrix[i,1],"Symbol"],])
}
...
barplot(Matrix[,19], ylim=c(0,2), las=1, main = "Induced genes > Induced interactions", col=col_vector)
...
barplot(Matrix[,20], ylim=c(-2,0), las=1, main = "Induced genes > Repressed interactions", col=col_vector)
...
barplot(Matrix[,21], ylim=c(-2,0), las=1, main = "Repressed genes > Induced interactions", col=col_vector)
...
barplot(Matrix[,22], ylim=c(0,2), las=1, main = "Repressed genes > Repressed interactions", col=col_vector)
```
