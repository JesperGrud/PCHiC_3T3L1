...
# Get only dynamic interactions
Overlap2 <- Overlap2[ Overlap2$Response != 0,]

# For each factor calculate the fractions
Direction <- data.frame(matrix(ncol=3,nrow=9))
for (i in c(1:9)) {
	Factor <- colnames(Counts)[(i+1)]
	TestPeaks$Zero_1 <- regexpr(paste(as.character(Factor),"_D0_E1",sep=""), TestPeaks$Factors)
	TestPeaks$Zero_2 <- regexpr(paste(as.character(Factor),"_D0_E2",sep=""), TestPeaks$Factors)
	...
	Direction[i,2] <- nrow(Tmp[ abs(Tmp[,2]) < log2(1.5),])/nrow(Overlap2)
	Direction[i,3] <- nrow(Tmp[ abs(Tmp[,2]) >= log2(1.5),])/nrow(Overlap2)
	}

# Plot it
barplot(t(as.matrix(Direction[c(7,6,5,4,1,3,2,8,9),c(2,3)])), beside=T, las=2, col=c("grey","darkgreen"), names=Direction[c(7,6,5,4,1,3,2,8,9),1])
```
