...
# Make a data frame to catch summary results
Result <- data.frame(matrix(ncol=5, nrow=9))

# Make a list to catch each ROC analysis
ROCs <- list()

# Perform ROC analysis for each point factor in a loop
for (i in c(1:9)) {
	# Get only peaks where the factor has been peak detected in both replicates in either D0 or H4 samples
	TestPeaks <- Peaks
	...
	Tmp <- Counts[ Counts$PeakID %in% TestPeaks[ (TestPeaks$Zero_1 > -1 & TestPeaks$Zero_2 > -1) | (TestPeaks$Four_1 > -1 & TestPeaks$Four_2 > -1) ,1],]
	# Merge interaction data and log2 fold changes of the factor
	Tmp2 <- merge(Overlap, Tmp[,c(1,(i+1))], by="PeakID")
	# Keep only induced and repressed interactions
	Tmp2 <- Tmp2[ Tmp2$Response == 1 | Tmp2$Response == -1,]
	# Perform ROC analysis
	ROCs[[i]] <- roc(Tmp2$Response, Tmp2[,6], direction = "<")
	# Save the factor name into summary results
	Result[i,1] <- Factor
	# Extract the AUC 
	Result[i,2] <- ROCs[[i]]$auc 
	# Extract the confidence levels
	Result[i,3] <- ci.auc(ROCs[[i]], conf.level=0.95)[1]
	Result[i,4] <- ci.auc(ROCs[[i]], conf.level=0.95)[3]
	# Save the number of sites bound by the factor
	Result[i,5] <- nrow(Tmp2)
}

# Set column names
colnames(Result) <- c("Factor","AUC","AUC_lower","AUC_upper","PeakCount")

# Order the data frame by AUC and define a color and an ID
Result <- Result[ order(-Result[,2]),]
Result$Color <- colorRampPalette(c("darkgreen","green","red"))(9)
...
# Plot the ROC curves
plot(1-ROCs[[as.numeric(Result[1,"ID"])]]$specificities, ROCs[[as.numeric(Result[1,"ID"])]]$sensitivities, type="l", xlab="False positive rate", ylab="True positive rate", xlim=c(0,1), ylim=c(0,1), col=Result[1,"Color"], las=1)
for (i in 2:9) { lines(1-ROCs[[as.numeric(Result[i,"ID"])]]$specificities, ROCs[[as.numeric(Result[i,"ID"])]]$sensitivities, col=Result[i,"Color"]) }
abline(0,1,lty=2)
```
