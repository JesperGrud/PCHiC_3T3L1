...
# Correlation between D0 boundaries and the rest
for (i in 1:nrow(Result)) {
  Result[i,157] <- cor(as.numeric(Result[i,c(1:31)]),as.numeric(Result[i,c(32:62)]))
  Result[i,158] <- cor(as.numeric(Result[i,c(1:31)]),as.numeric(Result[i,c(63:93)]))
  Result[i,159] <- cor(as.numeric(Result[i,c(1:31)]),as.numeric(Result[i,c(94:124)]))
  Result[i,160] <- cor(as.numeric(Result[i,c(1:31)]),as.numeric(Result[i,c(125:155)]))
}

# Plot the boxplot
boxplot(Result[,c(157:160)], outline=F, ylim=c(0,1), col=c("green3","green3","orange","blue3"), las=1, ylab="Pearson's R2 vs D0", names=c("4h","D2","ESC","Cortex"))
```
